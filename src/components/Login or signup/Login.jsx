...
const Login = () => {
    ...
    const addUser = async (e) => {
        ...
        try {
            ...
            Swal.fire({
                title: "Succes",
                text: "Sign up successfull please login",
                icon: "sucess"
            });
            setAction("Login");
        } catch (err) {
    ...
    const checkLogin = async (e) => {
        ...
        try {
            const res = await axios.get(`${API_USER}?email=${email}`);
            const user = res.data[0]; // JSON server return an array
            ...
            if (user.blocked) {
                Swal.fire("Blocked!","Your account has been blocked. Please contact support","error");
                return;
            }
